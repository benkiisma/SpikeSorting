...
clearvars channel count remove Waveform_inter
%% adjacency matrix
...
% Applying PCA to reduce the dimensions (Not using it in the next steps)

%PCA_results = cell(length(adj_mat),1);

%for unit=1:length(adj_mat)
...
%    [coeff,score,latent] = pca(X);
    %Take the most relevant PC
%    j=0;
%    inter = [];
%    while sum(inter) < 0.8*sum(latent) % 80% of the variance
%        j=j+1;
%        inter = latent(1:j);
%    end   
...
%    PCA_results{unit,1} = X*coeff;
%end

%clearvars X unit 
%% Clustering Running K-means

nb_iter = 10; %Setting the number of clustering iteration
...
clearvars count i Units_inter_copy off j Merged
